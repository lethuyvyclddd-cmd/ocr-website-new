...
use App\OCR\OcrCorrector;
use App\OCR\TextCleaner;
...
class FileTextExtractorService
{
    ...
    public function extract(string $filePath, string $extension): string
    {
        return match (strtolower($extension)) {
            'jpg', 'jpeg', 'png', 'bmp', 'webp' => $this->cleanOcrText($this->ocrService->read($filePath)['raw_text'] ?? ''),
    ...
    /**
     * Dọn rác + sửa các lỗi đọc sai điển hình của OCR (mất dấu, nhầm ký tự...)
     * trước khi đưa cho các Extractor bóc tách theo nhãn/regex.
     * CHỈ áp dụng cho text lấy từ OCR (ảnh / PDF scan) — không áp dụng cho
     * docx (chữ gõ thật, không qua OCR, vốn đã đúng/sạch sẵn).
     */
    protected function cleanOcrText(string $text): string
    {
        return OcrCorrector::correct(TextCleaner::clean($text)) ?? '';
    }
    ...
    protected function extractPdf(string $path): string
    {
        ...
        return $this->cleanOcrText($this->ocrService->readPdfScanned($path));
    }
}