            ...
            <ul class="list-unstyled">
                @foreach($applicant->documents as $doc)
                    <li class="d-flex align-items-center mb-1">
                        ...
                        <span class="text-muted ms-1">— {{ $doc->created_at->format('d/m/Y H:i') }}</span>
                        <a href="{{ asset('storage/' . $doc->image_path) }}" target="_blank" class="ms-2">Xem ảnh</a>

                        <form action="{{ route('applicants.documents.destroy', [$applicant->id, $doc->id]) }}"
                              method="POST" class="ms-2 d-inline"
                              onsubmit="return confirm('Xoá ảnh này? Dữ liệu OCR đã đọc từ ảnh này cũng sẽ mất.');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-outline-danger">🗑️ Xoá ảnh</button>
                        </form>
                    </li>
                @endforeach
            </ul>
